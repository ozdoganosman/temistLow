...
import type { AvgReturnChartData } from './deriveBacktestData';
...
interface Props {
  data: AvgReturnChartData;
  signalType: 'bullish' | 'bearish';
}

export default function AvgReturnChart({ data, signalType }: Props) {
  ...
  useEffect(() => {
    ...
    const title = signalType === 'bullish' ? 'ORT. GETIRI — AL' : 'ORT. GETIRI — SAT';
    ...
    const series = data.periods.map((p) => ({
      ...
      data: source[p] ?? [],
      itemStyle: {
        color: (params: any) => (params.value >= 0 ? (PERIOD_COLORS[p] ?? '#26a69a') : '#ef5350'),
      },
      barGap: '10%',
    }));
    ...
    chart.setOption(
      {
        ...
        tooltip: {
          ...
          formatter: (params: any) => {
            const lines = params.map((p: any) => `${p.marker} ${p.seriesName}: <b>${p.value.toFixed(2)}%</b>`);
